...
void addNoCheck_ht(SerialHashTable_t * htable,int key, volatile Packet_t * x, unsigned long long* remsumsum){
    ...
    if( htable->table[index] == NULL )
    	...
    	addNoCheck_list(htable->table[index],key,x,remsumsum);
}
...
void resize_ht(SerialHashTable_t * htable){
    ...
    for( int i = 0; i < htable->size; i++ ) {
          ...
          newTable[curr->key & ((2*htable->mask)+1)]->size++;
				  curr=nextnode;
			  }
        free(htable->table[i]);
		  }
